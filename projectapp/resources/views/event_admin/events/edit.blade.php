...
@section('content') 
<?php
$tabArr=["details","add_fields","interview_questions","hr_manager","schedule"];
$activeTab="details";
...
?>
<div class="cstm_event_admin_edit cstm_common_admin">
	<div class="container">
		<div class="row">
					  ...
					  <div class="card">
							...
							<div class="card-body">
								...
								<div class="row mb-3">
									...
									<div class="col-md-6">
									<input type="text" name="start_date" id="start_date" class="form-control" placeholder="Event Start Date" value="@if(isset($event)){{ date('m/d/Y',strtotime($event->start_date)) }}@else{{ old('start_date') ??  old('start_date') ?? date('m/d/Y') }}@endif" autocomplete="off" disabled>
									@error('start_date')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="row mb-3">
									...
									<div class="col-md-6">
									<input type="text" name="end_date" id="end_date" class="form-control" placeholder="Event End Date" value="@if(isset($event)){{ date('m/d/Y',strtotime($event->end_date)) }}@else{{ old('end_date') ??  old('end_date') ?? date('m/d/Y')  }}@endif"  autocomplete="off" disabled>
									@error('end_date')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
							...
							<div class="card-body">
								  ...
								  <div class="col-sm-12">
									 <div class="accordion" id="hr_accordion">
									  <a href="javascript:void(0)" class="hr_manageraccordian_tab @if( $activeTab!='hr_manager') collapsed @endif"  type="button" data-toggle="collapse" data-target="#hr_collapse" aria-expanded="false" aria-controls="hr_collapse">
										  Add Hr Manager
									  </a>
										<div id="hr_collapse" class="collapse @if( $activeTab=='hr_manager') show @endif" aria-labelledby="headingThree" data-parent="#hr_accordion">
										...
										<div id="schedule_collapse" class="collapse @if( $activeTab=='schedule') show @endif" aria-labelledby="headingThree" data-parent="#schedule_accordion">
											 <form method="POST" action="{{route('admin.schedule',['param'=>'save'])}}" >
												...
												<div class="row mb-3">
													...
													<div class="col-md-6">
														<input id="location_link" type="text" class="form-control @error('location_link') is-invalid @enderror" name="location_link" value="{{ old('location_link') }}" placeholder="Add Google Map embeded Url"  autocomplete="location_link" autofocus>
...
$interview_questionsData=[];
if(isset($event->interview_questions) && !empty($event->interview_questions)){
	$interview_questionsData=json_encode(unserialize($event->interview_questions));
?>
<script>
    jQuery(function() {

  var code = document.getElementById("event-interview_questions-build-wrap");
  var formData ='<?php echo $interview_questionsData ?>';	
   var addLineBreaks = html => html.replace(new RegExp("><", "g"), ">\n<");
