...
?>
<div class="cstm_events_admin_applicantinfo cstm_common_admin">
		   ...
		   <div class="card">
                <div class="card-header">RegNo-@if(isset($applicant)) <span class="text-info event_labelname">#{{ $applicant->uev_id }}</span> @endif 
				 @if(isset($applicant)) 
					...
					?>
					@if(isset($applicant->uev_status)) <span class="badge badge-info">{{$status[$applicant->uev_status]}}</span>@endif
				@endif
				 ...
				 <div class="card-body">
					@if(isset($applicant) && !empty($applicant))
						...
						<div class="col-md-6 grid-margin stretch-card">
							...
							<div class="col-md-3 grid-margin stretch-card">
									<div class="card-body">
									  <h4 class="card-title">Passport Number</h4>
									  <div class="media">
										<div class="media-body">
										  <p class="card-text">
										   {{ $applicant->passport }}
							...
							<div class="col-md-3 grid-margin stretch-card">
									<div class="card-body">
									  <h4 class="card-title">Current Visa Status</h4>
									  <div class="media">
										<div class="media-body">
										  <p class="card-text">
										  <?php
										  $visaStatusArr=get_visa_Status();
										  ?>
										   {{ $visaStatusArr[$applicant->visaStatus] ?? "n/a" }}
										  </p>
										</div>
									  </div>
									</div>
		  ...
		  <div class="form-group">
			...
			<select class="form-control evStatus" name="evStatus">
				...
				@if(in_array($k,$hiddenStatus))
					@continue
				@endif
					<option value="{{$k}}" @if( $applicant->uev_status==$k) selected @endif  @if($k=="6" && in_array($applicant->uev_status,$user_response_status))  selected @endif>{{$v}}</option>
				@endforeach
			</select>
		  </div> 
